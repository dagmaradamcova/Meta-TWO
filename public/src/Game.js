...
MetaTWO.Game.prototype = {
  create: function () {
    ...
    this.gameNumberDisplay = MetaTWO.game.add.text(370, 30, "game#", {
      font: "18px Arial",
      fill: "#ffffff",
      align: "right",
    });
    ...
    this.counterRotateKey = MetaTWO.game.input.keyboard.addKey(
      Phaser.Keyboard.K
    );
    this.pauseKey = MetaTWO.game.input.keyboard.addKey(Phaser.Keyboard.P);

    //  Stop the following keys from propagating up to the browser
    MetaTWO.game.input.keyboard.addKeyCapture([
      Phaser.Keyboard.A,
      Phaser.Keyboard.D,
      Phaser.Keyboard.S,
      Phaser.Keyboard.K,
  ...
  control: function () {
    ...
    if (this.softdrop_timer >= 0) {
      if (this.softdrop_timer === 0) {
        if (this.leftCurr || this.rightCurr) {
        } //do nothing - "regular" gravity
  ...
  gravity: function () {
    ...
    if (
      this.vy !== 0 ||
      this.drop >= this.speedLevels[this.level < 29 ? this.level : 29]
    ) {
      if (this.vy !== 0) {
        this.logEvent("ZOID", "U-DOWN", "");
      } else {
        this.logEvent("ZOID", "DOWN", "");
      }
      this.vy = 0;
      this.drop = 0;
  ...
  sub_94ee: function () {
    if (this.currentTask === this.lineAnim) {
        ...
        for (i = 0; i < this.rowsToClear.length; i++) {
          switch (this.are) {
            case 1:
              ...
              curDestroy.addMultiple(
                this.renderClear.getAll("x", 4 * 25 + 276)
              );
              curDestroy.addMultiple(
                this.renderClear.getAll("x", 5 * 25 + 276)
              );
            ...
            case 2:
              ...
              if (this.rowsToClear.length === 4) {
                this.stage.backgroundColor = 0xffffff;
              }
              curDestroy.addMultiple(
                this.renderClear.getAll("x", 3 * 25 + 276)
              );
              curDestroy.addMultiple(
                this.renderClear.getAll("x", 6 * 25 + 276)
              );
              curDestroy.callAll("kill");
              curDestroy.moveAll(this.zoidPrims);
            ...
            case 3:
              ...
              this.stage.backgroundColor = 0x050505;
              curDestroy.addMultiple(
                this.renderClear.getAll("x", 2 * 25 + 276)
              );
              curDestroy.addMultiple(
                this.renderClear.getAll("x", 7 * 25 + 276)
              );
              curDestroy.callAll("kill");
              curDestroy.moveAll(this.zoidPrims);
            ...
            case 4:
              ...
              curDestroy.addMultiple(
                this.renderClear.getAll("x", 1 * 25 + 276)
              );
              curDestroy.addMultiple(
                this.renderClear.getAll("x", 8 * 25 + 276)
              );
              curDestroy.callAll("kill");
              curDestroy.moveAll(this.zoidPrims);
      ...
      if (this.are >= this.LINECLEAR_STEPS) {
        ...
        this.rowsToClear = [];
        this.stage.backgroundColor = 0x050505;
        this.board.contents = JSON.parse(
          JSON.stringify(this.dummyBoard.contents)
        ); //animation is done, copy cleared lines to board for rendering
      }
      this._49 = 0;
    } else {
      for (i = 0; i < 4; i++) {
  ...
  whiteHighlight: function (x, y, offsetx, offsety) {
    MetaTWO.game.debug.geom(
      new Phaser.Rectangle(x + offsetx, y + offsety, 3, 3),
      "rgba(255,255,255,1)"
    );
    MetaTWO.game.debug.geom(
      new Phaser.Rectangle(x + offsetx + 3, y + offsety + 3, 6, 3),
      "rgba(255,255,255,1)"
    );
    MetaTWO.game.debug.geom(
      new Phaser.Rectangle(x + offsetx + 3, y + offsety + 6, 3, 3),
      "rgba(255,255,255,1)"
    );
  },
  ...
  poll: function () {
    ...
    if (
      MetaTWO.game.input.gamepad.supported &&
      MetaTWO.game.input.gamepad.active &&
      ...
      if (MetaTWO.config.pad === "axis") {
        if (this.downCurr) {
          this.downButton =
            this.gamepad.axis(Phaser.Gamepad.AXIS_1) >
            MetaTWO.config.releaseSensitivity;
        } else {
          this.downButton =
            this.gamepad.axis(Phaser.Gamepad.AXIS_1) >
            MetaTWO.config.pressSensitivity;
        }
        if (this.leftCurr) {
          this.leftButton =
            this.gamepad.axis(Phaser.Gamepad.AXIS_0) <
            -MetaTWO.config.releaseSensitivity;
        } else {
          this.leftButton =
            this.gamepad.axis(Phaser.Gamepad.AXIS_0) <
    ...
    if (this.leftCurr && !this.leftPrev) {
      this.logEvent("KEYPRESS", "PRESS", "LEFT");
    }
    if (!this.leftCurr && this.leftPrev) {
      this.logEvent("KEYPRESS", "RELEASE", "LEFT");
    }
    if (this.rightCurr && !this.rightPrev) {
      this.logEvent("KEYPRESS", "PRESS", "RIGHT");
    }
    if (!this.rightCurr && this.rightPrev) {
      this.logEvent("KEYPRESS", "RELEASE", "RIGHT");
    }
    if (this.downCurr && !this.downPrev) {
      this.logEvent("KEYPRESS", "PRESS", "DOWN");
    }
    if (!this.downCurr && this.downPrev) {
      this.logEvent("KEYPRESS", "RELEASE", "DOWN");
    }
    if (this.rotateCurr && !this.rotatePrev) {
      this.logEvent("KEYPRESS", "PRESS", "A");
    }
    if (!this.rotateCurr && this.rotatePrev) {
      this.logEvent("KEYPRESS", "RELEASE", "A");
    }
    if (this.counterRotateCurr && !this.counterRotatePrev) {
      this.logEvent("KEYPRESS", "PRESS", "B");
    }
    if (!this.counterRotateCurr && this.counterRotatePrev) {
      this.logEvent("KEYPRESS", "RELEASE", "B");
    }
    if (this.pauseCurr && !this.pausePrev) {
      this.logEvent("KEYPRESS", "PRESS", "PAUSE");
    }
    if (!this.pauseCurr && this.pausePrev) {
      this.logEvent("KEYPRESS", "RELEASE", "PAUSE");
    }
  },
  ...
  onlyDownHit: function () {
    ...
    if (
      this.justPressed(this.keys.DOWN) &&
      !this.justPressed(this.keys.LEFT) &&
      !this.justPressed(this.keys.RIGHT) &&
    ...
    if (MetaTWO.config.logging == true) {
      ...
      let logit = function (val, key) {
        if (loglist.indexOf(key) !== -1) {
          // if key is present in loglist
          data.push(val); // push value
        } else {
          // else (if key is not present in loglist)
          data.push(""); // push empty string
        }
      };

      data.push(MetaTWO.game.time.totalElapsedSeconds()); // seconds elapsed since game started
      data.push(event_type);
      logit(MetaTWO.config.subjectNumber, "SID"); // subject ID
      ...
      logit(complete, "completed"); // set to false?, only logged in logGameSumm
      logit(
        MetaTWO.game.time.totalElapsedSeconds() - this.gameStartTime,
        "game_duration"
      ); // only logged in logGameSumm
      logit(
        (MetaTWO.game.time.totalElapsedSeconds() - this.gameStartTime) /
          (this.episode + 1),
        "avg_ep_duration"
      ...
      logit(JSON.stringify(this.zoidBuff), "zoid_sequence"); // only logged in logGameSumm
  ...
  logEvent: function (id, evt_data1, evt_data2) {
    if (MetaTWO.config.logging == true) {
      ...
      this.logUniversal("GAME_EVENT", loglist, {
        evt_id: id,
        evt_data1: evt_data1,
        evt_data2: evt_data2,
      });
